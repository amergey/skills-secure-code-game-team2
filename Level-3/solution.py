...
character set allowed to alphanumeric, but sometimes this approach is too 
restrictive. We might need to handle arbitrary filenames or the
code needs to run cross-platform and account for filesystem differences between
Windows, Macs and *nix.
...
The user input can be later appended to the safely generated base directory
so that the absolute filepath is normalized. 

Finally, add a check on the longest common subpath between the
base directory and the normalized filepath to make sure that no traversal 
is about to happen and that the final path ends up in the intended directory.

The GitHub Security Lab covered this flaw in one episode of Security Bites, 
its series on secure programming: https://youtu.be/sQGxdwRePh8
